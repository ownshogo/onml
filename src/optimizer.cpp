...
float
sgd::next_bias(float current_bias, float y_gradient)
{
  return current_bias - this->learning_rate * y_gradient;
}

Eigen::VectorXf
sgd::next_weights(const Eigen::VectorXf& current_weights,
                  const Eigen::VectorXf& weights_gradient)
{
  return current_weights - this->learning_rate * weights_gradient;
}

adagrad::adagrad(float initial_learning_rate, const std::size_t dim)
...
float
adagrad::next_bias(float current_bias, float y_gradient)
{
  this->bias_r += y_gradient * y_gradient;
  this->bias_learning_rate =
    this->initial_learning_rate / std::sqrt(this->bias_r);
  return current_bias - this->bias_learning_rate * y_gradient;
}

Eigen::VectorXf
adagrad::next_weights(const Eigen::VectorXf& current_weights,
                      const Eigen::VectorXf& weights_gradient)
{
